...
int main(int argc, char *argv[])
{
  ...
  // Prepating size and state variables
  int c;
  int n = (1<<20);  
  int *in, *out;
  int *c_scan;
  ...
  // Split computation 32 times because 32 bits system.
  for (int k = 0 ; k < 32; k++)
  {
    // Split part is in the scan so it is not necessary here
    /* compute indices of element swith 0 in lsb */
    ...
    /* compute indices of element swith 1 in lsb */
    rsort_scan(cv.commands,
      cv.context,
      kernel_map[scan_name_str],
      kernel_map[update_name_str],
    ...
    // Scatter part and reassemblying part
